...
export interface SonarGridProps extends React.ComponentProps<"div"> {
  ...
  baseOpacity?: number
  /** Any CSS color. Defaults to the theme's accent color, so it adapts to light/dark and brand themes. */
  color?: string
 ...
 * Canvas-based and theme-aware (it reads the resolved `text-accent` color), it idles
...
export function SonarGrid({
  ...
  // Synced in an effect (not during render) to satisfy the react-hooks/refs rule.
  ...
  return (
    ...
    >
      <canvas
        ref={canvasRef}
        aria-hidden="true"
        className="text-accent pointer-events-none absolute inset-0 -z-10 size-full"
